...
class RiverTracer:
	...
	def __init__(self, architecture, symbolized, maxInputSize, targetAddressToReach = None):
		...
		# 	# self.ctx.enalbleMode(MODE.SYMBOLIZE,True)
		# assert self.context.isModeEnabled() == symbolized
	...
	def loadBinary(tracersInstances, binaryPath, entryfuncName):
		...
		for tracerIndex, tracer in enumerate(tracersInstances):
			...
			for phdr in phdrs:
				...
				tracersInstances[tracerIndex].context.setConcreteMemoryAreaValue(vaddr, phdr.content.tolist())
